...
| **P1 · 墙编排骨架** | `wall/detector.js` + `wall-router.js` + P/A 策略骨架；种子站点 x.com 全墙判定 | 五墙检测报告正确；付费墙降级提示可达 |
| **P2 · P 墙快照闭环** | archive.is 快照抓取 + 正文解析 + 失败降级 | Substack 付费文一键得干净正文可译；快照失败有清晰提示 |
